...
/**
 * Handles parsing of the image clip function in the Picasso language.
 * 
 * @author Ford Scott
 */
public class ImageClipAnalyzer implements SemanticAnalyzerInterface {
    ...
    /**
     * Generates an expression tree node for the image clip function.
     * 
     * @param tokens the stack of tokens, which should contain the image clip parameters
     * @return an ImageClip expression tree node
     * @throws ParseException if the correct tokens are not found
     */
    @Override
    public ExpressionTreeNode generateExpressionTree(Stack<Token> tokens) {
        tokens.pop(); // Pop the ImageClip function token
        ...
        if (tokens.peek() instanceof ImageToken) {
            ...
            return new ImageClip((Image) imageNode, paramx, paramy);
        } else {
